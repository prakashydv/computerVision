...
OCglobal=[350.804600;272.529477];%28xCover
OCmap=[351.559594,274.257778];
...
for ii=1:m
    for jj=1:2:n
        ...
        %fprintf('[%d][%d] rmsErr : %f\n',ii,uint8(jj/2),rms(Err(1:ErrCount,1)));
        countErr=countErr+1;ERR(countErr,1)=rms(Err(1:ErrCount,1));
        heatmap(ii,jj/2+0.5)=1-(rms(Err(1:ErrCount,1))/100);
    end
end
%heatmap;
fprintf('\nmean:%f\tstdDev:%f\tmaxErr:%f\n',mean(ERR),std(ERR),max(ERR));
imtool(heatmap);
%imwrite(heatmap,'heatmapp.jpg');
